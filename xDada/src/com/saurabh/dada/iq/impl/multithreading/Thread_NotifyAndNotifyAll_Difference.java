...
public class Thread_NotifyAndNotifyAll_Difference {
	...
	public static void main(String[] args) {
		try{
			...
			t1.start();
			t2.start();
			Thread.sleep(100); // Used to ensure that thread1 and thread2 starts
								// before thread-3
			// because thread-1 and 2 calls wait(), while thread-3 calls notify
			// or notifyAll()
			t3.start();
...
class MyRunnable2 extends Thread {
	...
	public MyRunnable2(MyRunnable1 MyRunnable1) {
...
class MyRunnable3 extends Thread {
	...
	public MyRunnable3(MyRunnable1 MyRunnable1) {
		this.myRunnable1 = MyRunnable1;
	}
	...
	public void run() {
		...
		synchronized (this.myRunnable1) {
			System.out.println(Thread.currentThread().getName() + " started");
			// this.myRunnable1.notify(); // Wakes up a single thread that is waiting on this object's monitor.
			// If any threads are waiting on this object, one of them is chosen to be awakened.
			// The choice is random and occurs at the discretion of the implementation.

			this.myRunnable1.notifyAll(); // Will wake up all threads waiting on object's monitor.
